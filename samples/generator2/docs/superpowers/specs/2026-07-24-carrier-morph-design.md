...
```
steps = max(1, round(T * sampleRate))       // morphSteps(), §5.2
inc   = max(1, 2^32 / steps)                // прирост фазы шага на сэмпл
morph_phase += inc                          // каждый сэмпл; переполнение = граница шага
```
...
`fade_samples = 240` (5 мс при 48 кГц), зажимается сверху длиной шага. Минимальное `T` = 0.1 с =
4800 сэмплов, так что зажим не срабатывает никогда — он на случай смены частоты дискретизации.
...
| Целевой слот погас на ходу | Пара `(slot, slot_next)` фиксирована на весь шаг, поэтому `pB` не подменяется — щелчка нет. Шаг доигрывает в старую цель, на границе цикл берёт актуальную маску. Самовосстановление за один шаг. |
...
| `T` меняется на ходу | Меняется только `inc`; `morph_phase` сохраняется, поэтому позиция внутри шага и `t` не прыгают (§4.1). Безопасно даже при перетаскивании регулятора. |
