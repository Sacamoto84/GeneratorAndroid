...
Логика не меняется нигде. Правятся только три строки включений, и все три
становятся короче — файлы съезжаются в одну папку, а не расходятся.

JNI-имена от расположения файла не зависят, поэтому набор экспортируемых
символов обязан остаться прежним.
...
## Что происходит с включениями

`#include "x.h"` ищет файл сначала рядом с включающим, поэтому подавляющее
большинство включений продолжит работать без правок: группы совпали с
фактическими кластерами зависимостей. `plasma.cpp` включает `fft.h`,
`colormaps.h`, `waterfall.h`, `scale.h`, `auformat.h`, `ScaleBufferBase.h`,
`ScaleBuffer.h` и `BufferAverage.h` — все они едут в `spectrogram/` вместе с
ним.

Правки требуют ровно три файла, и все три — там, где путь сейчас пересекает
границу `route/`, а после перекладки перестанет:

| Файл | Сейчас | Станет |
|---|---|---|
| `generator2.cpp` | `"route/FloatRingBuffer.h"` | `"FloatRingBuffer.h"` |
| `plasma.cpp` | `"route/global.h"` | `"global.h"` |
| `jniFFT.cpp` | `"../fft.h"` и ещё семь таких же | `"fft.h"` и так далее |

Единственное включение, пересекающее границу папки и после перекладки, —
`audio_common.h`, который спектрограмма тянет из `audio/`. Оно покрыто строкой
`INCLUDE_DIRECTORIES(audio)` в `CMakeLists.txt` и правки не требует.

Отдельный случай — `minidsp.c` с его `#include "api/fftw3.h"`. Путь сломается,
но файл уезжает в `attic/`, не входит в сборку и никем не включается, поэтому
последствий нет.
...
**Перемещаются** — 33 файла по таблице структуры выше, через `git mv`, чтобы
`git log --follow` продолжал показывать историю каждого.

**Изменяются**

- `app/src/main/cpp/CMakeLists.txt` — пути в списке исходников получают префикс
  папки, `generator2.cpp` меняется на `scope/nativeLib.cpp`, закомментированные
  строки про `route/` удаляются вместе с самой папкой.
- `scope/nativeLib.cpp`, `spectrogram/plasma.cpp`, `spectrogram/jniFFT.cpp` —
  по таблице включений выше.
