...
public:
     ...
     * при развёртке меньше 32. Начало окна выбирает вызывающая сторона:
     * оно уже выровнено по фронту синхронизации.
     */
...
private:
    ...
    void advanceTo(int column) {
        ...
        // Столбец, который голова покидает, только сейчас заполнен целиком.
        // Пометить его заново обязательно: грязным он был отмечен при заходе,
        // то есть пустым, и если поток GL забрал диапазон между заходом и
        // уходом, в текстуру попала пустая версия. Второй пометки не было бы —
        // и она осталась бы там навсегда.
        markDirty(lastColumn_);
