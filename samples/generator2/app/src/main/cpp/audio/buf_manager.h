...
class ProducerConsumerQueue {
 public:
   ...
   * если очередь пуста, то есть если нет доступных элементов для чтения.
   * В этом случае лямбда-выражение не будет вызвано, и значение out_item не будет изменено.
   * @param out_item
   * @return
   */
   ...
   */
  void pop() {
    int readptr = read_.load(std::memory_order_relaxed); //std::memory_order_relaxed позволяет выполнять загрузку без синхронизации с другими операциями
    ++readptr;
