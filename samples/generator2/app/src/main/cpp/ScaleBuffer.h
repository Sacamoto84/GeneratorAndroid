...
public:
    ...
    /**
     * Получить частоту для данного x координаты на экране
     * @param x
     * @return Частота
     */
    [[nodiscard]] float XtoFreq(float x) const override
    {
        return scaleXtoFreq.forward(x);
    }

    [[nodiscard]] float FreqToX(float freq) const override
    {
    ...
    void Build(BufferIODouble *inputIO, float volume) override
    {
