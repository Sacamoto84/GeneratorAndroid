...
class MyGLRenderer : GLSurfaceView.Renderer {
    ...
    void main() {
        ...
        float x = float(gl_VertexID)/100.0 - 1.0f;
...
""".trimIndent()
    ...
    override fun onDrawFrame(gl: GL10?) {
       // val nanos = measureNanoTime {
            ...
            val stepXHandle = GLES30.glGetUniformLocation(program, "stepX")
            val offsetXHandle = GLES30.glGetUniformLocation(program, "offsetX")
            val scaleYHandle = GLES30.glGetUniformLocation(program, "scaleY")
            val stepX = 2.0f / (vertexBuffer.limit() / 2)
            GLES30.glUniform1f(stepXHandle, stepX)
            GLES30.glUniform1f(offsetXHandle, -1.0f)
            GLES30.glUniform1f(scaleYHandle, 1.0f)
