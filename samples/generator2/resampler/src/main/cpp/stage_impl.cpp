...
//	printf("build filter %f %f %f, %f/%i %f (lenstep=%i)\n",fpass,fstop,fmax,rp,rpFactor, rs,lenStep);
	...
	*h = static_cast<double *>(malloc((*len) * sizeof(double)));
	for (i = 0; i < *len; i++)
		(*h)[i] = 0;
...
struct PSFilter* init_psfilter(int L, int M,
	...
	struct PSFilter* pfilt = static_cast<PSFilter *>(malloc(sizeof(struct PSFilter)));
	...
	if (Lenh > K*L)
		K++;
	pfilt->filters = static_cast<double *>(malloc(L * K * sizeof(double)));
